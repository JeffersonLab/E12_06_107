void replay_production_hms_ct(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/pass2/hms_coin_replay_production_%d_%d.root";
  ...
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/HMS/PRODUCTION/pass2/summary_coin_production_%d_%d.report", RunNumber, MaxEvent));    // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template.
  analyzer->PrintReport("TEMPLATES/HMS/PRODUCTION/hstackana_production.template",
			Form("REPORT_OUTPUT/HMS/PRODUCTION/pass2/replay_hms_coin_production_%d_%d.report", RunNumber, MaxEvent));
