void replay_production_ct (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/pass_/coin_replay_production_%d_%d.root";
  ...
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/COIN/PRODUCTION/pass2/summary_production_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production.template",
  			Form("REPORT_OUTPUT/COIN/PRODUCTION/pass2/replay_coin_production_%d_%d.report", RunNumber, MaxEvent));  // optional
